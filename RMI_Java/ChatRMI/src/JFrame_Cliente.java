    ...
    frase = jTextField_Mensagem.getText();
        if (!frase.isEmpty()) {
            try {
                ...
                String ipCliente = "IP desconhecido";
                try {
                    ipCliente = InetAddress.getLocalHost().getHostAddress();
                } catch (UnknownHostException e) {
                    System.err.println("Nao foi possivel obter o IP local: " + e.getMessage());
                }

                String mensagemCompleta = "[" + timestamp + "] [" + ipCliente + "] " + apelido + ": " + frase;
                chat.receberMensagem(mensagemCompleta);
