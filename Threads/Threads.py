...
def popular_lista_intervalo(lista, inicio, fim, max_lista):
    tamanho_intervalo = fim - inicio + 1
    if tamanho_intervalo <= max_lista:
        lista.extend(range(inicio, fim + 1))
    else:
        lista.extend(random.sample(range(inicio, fim + 1), max_lista))
...
def ordenar_pente(lista):
   n = len(lista)
    ...
    while distancia > 1 or houve_troca:
        distancia = int(distancia / fator_shrink)
        if distancia < 1:
            distancia = 1
        houve_troca = False
...
if __name__ == "__main__":
    ...
    t1 = threading.Thread(target=popular_lista_intervalo,args=(lista1, 3001, 10000, max_lista))
    t2 = threading.Thread(target=popular_lista_intervalo,args=(lista2, 0, 3000, max_lista))
